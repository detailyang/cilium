...
func (o *OpLabels) GetIdentityLabel(key string) (l Label, found bool) {
	l, found = o.orchestrationIdentity.labels[key]
	if !found {
...
func (o *OpLabels) ReplaceIdentityLabels(l Labels, logger *logrus.Entry) bool {
	...
	for k, v := range l {
		...
		if _, found := o.disabled.labels[k]; found {
...
func (o *OpLabels) ModifyIdentityLabels(addLabels, delLabels Labels) (changed bool, err error) {
	for k := range delLabels {
		...
		if _, found := o.custom[k]; !found {
			if _, found := o.orchestrationIdentity.labels[k]; !found {
				if _, found := o.disabled.labels[k]; !found {
	...
	for k := range delLabels {
		if v, found := o.orchestrationIdentity.labels[k]; found {
	...
	for k, v := range addLabels {
		if _, found := o.disabled.labels[k]; found { // Restore label.
			delete(o.disabled.labels, k)
			o.orchestrationIdentity.labels[k] = v
			changed = true
		} else if _, found := o.orchestrationIdentity.labels[k]; found { // Replace label's source and value.
			o.orchestrationIdentity.labels[k] = v
			changed = true
		} else {
			o.custom[k] = v
